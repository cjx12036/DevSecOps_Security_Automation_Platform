...
## 功能概览
- 通过 `secscan agent` 做并发编排扫描（SAST + Secrets）。
- 外部扫描后端：Semgrep（SAST）、Trivy（Secrets），并保留本地规则回退。
...
make scan
```
`make setup` 会安装 Semgrep。CI 中会安装 Trivy；本地若未安装 Trivy，Secrets 会自动回退到内置规则引擎。
`make scan` 总是本地生成报告产物；本地策略门禁请执行：
```bash
make gate
```
...
- 执行：`secscan agent`（并发调度 Semgrep + Trivy，缺失时自动回退）。
- 上传：SARIF 到 GitHub Code Scanning，分两个 category：
  - `secscan-sast`
  - `secscan-secrets`
- 门禁：执行策略检查，高/严重漏洞使流水线失败。

## 如何新增规则
1. 在 `secscan/rules/semgrep/sast.yml` 新增 Semgrep 规则，或在 `secscan/rules/` 增加 Python 本地规则。
2. 本地规则需在 `secscan/rules/__init__.py` 与 `secscan/scanners.py` 注册。
3. 在 `tests/` 增加检测与边界单测。
